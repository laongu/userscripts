...
async function handleButtonClick() {
        try {
            ...
            async function translateText(text) {
                try {
                    let splitText = dictionary.tokenize(text);

                    let translatedText = splitText.map(word => {
                        let translation = '';
                        let phienAm = [];
                        let weights = 0;
                        ...
                        return [word, translation, phienAm.join(' '), weights];
                    });
                    ...
                    return '<div>Đã xảy ra lỗi trong quá trình dịch.</div>';
                }
            }

            // Sử dụng hàm
            const translatedText = await translateText(originalTextContent);
...
async function displayResult(tuLap) {
    ...
    const tuLapTransArray = await translateText(tuLapString);
